...
class YahooScreener:
    def __init__(self):
        ...
        self.db_session = self.engine.sessionmaker()
        # 상장 종목 목록 가져오기
    ...
    def get_screener(self):
        ...
        for i in symbol_list:
            ...
            t = db.query(exists().where(KrCapRank.symbol == i['symbol'])).scalar()

            #print(t)
            if t:  # DB에 있다면
                sql_data = db.query(KrCapRank).where(KrCapRank.symbol == i['symbol']).first()
                sql_data.market_price = int(i['regularMarketPrice']['raw'])
                sql_data.market_change = int(i['regularMarketChange']['raw'])
                sql_data.market_change_percent = float(i['regularMarketChangePercent']['raw'])
                sql_data.shares_outstanding = int(i['sharesOutstanding']['raw'])
                sql_data.market_cap = int(market_cap)
            else:  # DB에 없다면
                sql_data = KrCapRank(
                    exchange=i['fullExchangeName'],
                    symbol=i['symbol'],
                    name=kr_name,
                    market_price=int(i['regularMarketPrice']['raw']),
                    market_change=int(i['regularMarketChange']['raw']),
                    market_change_percent=float(i['regularMarketChangePercent']['raw']),
                    shares_outstanding=int(i['sharesOutstanding']['raw']),
