...
country = {'USA': '미국', 'S. Arabia': '사우디아라비아', 'Taiwan': '대만', 'Denmark': '덴마크',
...
for d in data:
    ...
    t = session.query(exists().where(Stock_Rank.code == d[2])).scalar()
    print(t)

    if t:  # DB에 있다면
        stock_data = session.query(Stock_Rank).where(Stock_Rank.code == d[2]).first()
        stock_data.market_cap = d[3]
        stock_data.price = d[4]
        stock_data.day_change = d[5]
        stock_data.date = now_date
        stock_data.dollar = dollar
        session.commit()
    else:  # DB에 없다면
